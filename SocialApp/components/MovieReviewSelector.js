// components/MovieReviewSelector.js - Movie review search and selection
...
export default function MovieReviewSelector({ visible, onClose, onSelect }) {
  ...
  useEffect(() => {
    ...
    searchTimeoutRef.current = setTimeout(async () => {
      setLoading(true);
      try {
        const response = await api.get('/api/reviews/search-movies', {
  ...
  const handleConfirm = () => {
    if (selectedMovie && rating > 0) {
      onSelect({
        ...
        artist: null, // Will be filled from details
        ...
        genre: [],
        duration: null,
        externalUrl: `https://www.themoviedb.org/movie/${selectedMovie.id}`
      });
      handleClose();
    }
  };
    ...
    >
      <SafeAreaView style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>Review Movie</Text>
          ...
          <View style={styles.selectedContainer}>
            <View style={styles.selectedMovieCard}>
              {selectedMovie.poster && (
                <Image source={{ uri: selectedMovie.poster }} style={styles.selectedPoster} />
              )}
              <Text style={styles.selectedTitle}>{selectedMovie.title}</Text>
              {selectedMovie.year && (
                <Text style={styles.selectedYear}>{selectedMovie.year}</Text>
              )}
            </View>
